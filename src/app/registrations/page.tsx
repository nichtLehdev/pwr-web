...
export default function MyRegistrationsPage() {
  ...
  const canEdit = (registration: NonNullable<typeof data>["registrations"][0]) => {
    const now = new Date();
    ...
    if (courseStart <= now || registration.registrationStatus === RegistrationStatus.CANCELLED) {
      return false;
    }
  ...
  const getEditDeadlineInfo = (registration: NonNullable<typeof data>["registrations"][0]) => {
    const now = new Date();
    const courseStart = new Date(registration.course.startDate);
    const deadline = registration.course.registrationDeadline
      ? new Date(registration.course.registrationDeadline)
    ...
    if (registration.registrationStatus === RegistrationStatus.CANCELLED) {
      return { canEdit: false, message: "Stornierte Anmeldung", color: "text-gray-500" };
    }
    
    if (courseStart <= now) {
      return { canEdit: false, message: "Kurs hat bereits begonnen", color: "text-gray-500" };
    }
    
    if (deadline && deadline <= now) {
      return { canEdit: false, message: "Anmeldefrist abgelaufen", color: "text-red-600 dark:text-red-400" };
    }
    
    // Calculate days until deadline or course start
    const editUntil = deadline || courseStart;
    const daysUntil = Math.ceil((editUntil.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mb-4 space-y-2 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        {/* Edit deadline info */}
                        <div className={`flex items-center gap-2 ${editInfo.color}`}>
                          <svg
                            className="h-4 w-4"
                            fill="none"
                            stroke="currentColor"
                          ...
                          <span className="text-sm font-medium">{editInfo.message}</span>
                        </div>
                      </div>
                    ...
                    {/* Actions */}
                    <div className="flex flex-col gap-2 lg:items-stretch lg:min-w-[140px]">
                      <Link
        ...
        {data && data.pages > 1 && (
          <div className="mt-8 flex items-center justify-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="dark:border-dark-border dark:bg-dark-surface dark:text-dark-text rounded-lg border border-gray-300 bg-white px-4 py-2 font-medium transition-colors hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed dark:hover:bg-gray-800"
            >
              Zurück
            </button>
