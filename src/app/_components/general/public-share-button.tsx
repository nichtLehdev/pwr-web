...
async function buildShareQrDataUrl(payload: string, logoSrc: string): Promise<string> {
...
export default function PublicShareButton({
  ...
  useEffect(() => {
    ...
    const logoSrc = dark ? "/images/logo-icon-dark.svg" : "/images/logo-icon.svg";
  ...
  return (
    <>
      ...
      {shareOpen && typeof document !== "undefined"
        ? createPortal(
        <div className="fixed inset-0 z-[9999] isolate flex items-center justify-center bg-black/55 p-4">
                ...
                {hasNativeShare ? (
                  <div className="dark:border-dark-border flex overflow-hidden rounded-lg border border-gray-200">
                    <button
                      type="button"
                      onClick={() => void onCopy()}
                      className="hover:bg-gray-50 inline-flex min-w-0 flex-1 items-center justify-center gap-2 border-0 bg-transparent px-3 py-2 text-sm font-medium text-gray-700 transition-colors dark:text-gray-200 dark:hover:bg-gray-800/80"
                    >
                      {copied ? <CheckIcon className="h-4 w-4" /> : <CopyIcon className="h-4 w-4" />}
                      <span className="truncate">{copied ? "Kopiert" : "Link kopieren"}</span>
                    </button>
                    <button
                      type="button"
                      ...
                      className="dark:border-dark-border hover:bg-gray-50 inline-flex shrink-0 items-center border-0 border-l border-gray-200 bg-transparent px-2 py-2 text-gray-700 transition-colors dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800/80"
                  ...
                  <button
                    type="button"
                    onClick={() => void onCopy()}
                    className="dark:border-dark-border hover:bg-gray-50 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 transition-colors dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800/80"
                  >
                    {copied ? <CheckIcon className="h-4 w-4" /> : <CopyIcon className="h-4 w-4" />}
                ...
                {moreOptionsOpen && hasNativeShare ? (
                  ...
                  >
                    <button
                      ...
                      className="dark:hover:bg-gray-800 flex w-full items-center px-4 py-2 text-left text-sm text-gray-800 hover:bg-gray-50 dark:text-gray-100"
                    >
                      Über Gerät teilen&nbsp;…
                    </button>
                  </div>
            ...
            <div className="rounded-lg bg-white p-3">
              <div className="flex justify-center">
                {qrDataUrl ? (
                  ...
                  <img src={qrDataUrl} alt="QR-Code zum Teilen" className="h-56 w-56" />
                ) : (
              ...
              {qrDataUrl ? (
                <div className="mt-4 flex justify-center">
                  <button
                    ...
                    className="dark:border-dark-border hover:bg-gray-50 inline-flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 transition-colors dark:text-gray-200 dark:hover:bg-gray-800"
                  >
