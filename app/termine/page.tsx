...
export default function TerminePage() {
  ...
  const filteredItems = allItems.filter(item => {
    ...
    // District Filter
    if (selectedDistrict !== 'all' && item.districtInfo.name !== selectedDistrict) return false;

    // Category Filter
    if (selectedCategory !== 'all') {
      if (item.type === 'event' && item.category !== selectedCategory) return false;
      if (item.type === 'course' && item.courseType !== selectedCategory) return false;
    }
  ...
  const groupedByMonth = sortedItems.reduce((acc, item) => {
    const date = new Date(item.type === 'event' ? item.eventDate : item.startDate);
  ...
  const courseCategories = ['D-Course', 'C-Course', 'Workshop', 'Training', 'Other'];
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex-1 text-center">
              <span className="text-sm text-gray-600">
                {sortedItems.length} {sortedItems.length === 1 ? 'Termin' : 'Termine'}
                {(filterType !== 'all' || selectedDistrict !== 'all' || selectedCategory !== 'all') && (
        ...
        <div className="container mx-auto px-4">
          ...
          {viewMode === 'list' ? (
            ...
            <div className="space-y-8 md:space-y-12">
              {Object.entries(groupedByMonth).map(([monthKey, { label, items }]) => (
                <div key={monthKey}>
