...
export default function CompactEventCard({
  ...
  // Prüfe ob mehrtägig
  const isMultiDay = endDate && new Date(endDate).toDateString() !== dateObj.toDateString();
  ...
  return (
    ...
    >
      {/* Date Badge */}
      <div className={`flex-shrink-0 ${isMultiDay ? 'min-w-20' : 'w-12'} h-12 bg-gray-100 rounded-lg flex ${isMultiDay ? 'flex-row' : 'flex-col'} items-center justify-center gap-1 px-2`}>
        {isMultiDay ? (
          <>
